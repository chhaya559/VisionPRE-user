...
export const usePurchaseTicketBlockchain = () => {
  ...
  const purchaseTicketBlockchain = async (
    galaId: string,
    organiserWallet: string,
    price: number
  ) => {
    ...
    try {
      ...
      // 4. Check Balance
      setProcessStatus(
        t('subscription.processing.checkingToken') || 'Checking balance...'
      );
      ...
      if (errorMsg === 'ALREADY_PURCHASED') {
        ...
        const recoveredHash = await findPreviousPurchaseHash(
          normalizedGalaId,
          account || ''
        );
        ...
        if (recoveredHash) {
          toast.success(
            'Previous purchase found! Synchronizing with server...'
          );
          const normalizedAddress = account ? ethers.getAddress(account) : '';
          return {
        ...
        toast.error(
          'You are already registered on blockchain, but we could not find your transaction hash. Please contact support.'
        );
        return null;
      }
