...
export const useSubscription = () => {
  const { data: subscriptionResponse, isLoading, isError, refetch } = useGetSubscriptionQuery(undefined, {
  ...
  const isActive = subscription?.subscriptionStatus === 'Active' || subscription?.status === 1;
